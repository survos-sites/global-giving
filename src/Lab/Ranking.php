...
final class Ranking
{
    ...
    {
        $documents = []; $scores = []; $contributions = [];
        foreach (['keyword' => $keyword, 'vector' => $vector] as $kind => $list) {
            $seen = [];
            foreach ($list as $rank => $hit) {
                $id = (string) $hit['id'];
                if (isset($seen[$id])) { continue; }
                $seen[$id] = true; $documents[$id] = $hit;
                $contributions[$id][$kind.'Rank'] = $rank + 1;
                $contributions[$id][$kind.'Score'] = $hit['score'] ?? null;
                $scores[$id] = ($scores[$id] ?? 0.0) + 1 / (60 + $rank + 1);
            }
        }
        uksort($scores, static fn ($a, $b) => ($scores[$b] <=> $scores[$a]) ?: strcmp((string) $a, (string) $b));
        return array_map(static fn ($id) => [...$documents[$id], 'score' => $scores[$id], 'contributions' => $contributions[$id]], array_slice(array_keys($scores), 0, $limit));
    }
}
